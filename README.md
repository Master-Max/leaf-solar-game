...
**rising**. Climbing a beam, the air arrives from below, so the tilt that glided
you east on the way down carries you west on the way up. Getting height and
getting distance want opposite hands.

Every metre you travel has to be bought with height you earned in a beam. Going
edge-on multiplies your climb rate inside the light, but the same angle makes
you fall like a stone the moment you leave it.
...
- `PHYS.faceDrag` and `PHYS.edgeDrag` are the whole flight model. Force is
  applied along the blade's normal, so glide and sink both fall out of the one
  angle. The ratio between them sets the spread of sink rates; the absolute
